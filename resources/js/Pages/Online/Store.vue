    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-8">
      ...
      <div class="bg-gradient-to-r from-[#00360D] via-[#004D13] to-[#00661A] rounded-[2.5rem] p-6 sm:p-10 text-white shadow-xl relative overflow-hidden">
        <div class="relative z-10 max-w-2xl space-y-3">
          <span class="px-3.5 py-1.5 bg-white/10 backdrop-blur-md rounded-full text-[10px] font-black uppercase tracking-wider text-emerald-200 border border-white/10 inline-block">
            ⚡ Belanja Instan &bull; Vue 3 SPA Edition
          </span>
      ...
      <div class="bg-white p-4 sm:p-6 rounded-[2rem] border border-slate-100 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-4">
        ...
        >
          ...
          <span class="bg-white text-[#00880F] px-2 py-0.5 rounded-full text-[10px] font-black">{{ totalItems }}</span>
        </button>
      </div>
      ...
      <!-- ========================================================== -->
      <!-- MODAL DETAIL PRODUK & MULTI-FOTO CAROUSEL -->
      <!-- ========================================================== -->
      <div 
      ...
      >
        ...
        <div class="relative bg-white w-full max-w-3xl rounded-[2.5rem] sm:rounded-[3rem] shadow-2xl border border-slate-100 overflow-hidden z-10 my-auto max-h-[92vh] flex flex-col">
          ...
          <div class="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-white shrink-0">
            ...
            <button @click="closeProductModal" class="w-9 h-9 flex items-center justify-center text-slate-400 hover:text-slate-900 rounded-full hover:bg-slate-100 text-sm font-black transition">
              ✕
            </button>
          </div>
      ...
      <!-- ========================================================== -->
      <!-- DRAWER KERANJANG & CHECKOUT FORM -->
      <!-- ========================================================== -->
      <div 
        v-if="isCartOpen" 
          ...
          <div class="flex justify-between items-center p-5 sm:p-6 border-b border-slate-100 bg-white shrink-0">
            <div class="flex items-center space-x-2">
              ...
              <div>
                ...
                <p class="text-[10px] text-slate-400 font-bold mt-0.5">{{ totalItems }} Produk Dipilih</p>
              </div>
            </div>
            <button @click="isCartOpen = false" class="w-8 h-8 flex items-center justify-center text-slate-400 hover:text-slate-700 rounded-full hover:bg-slate-100 text-sm font-black transition">
              ✕
            </button>
          </div>
          ...
          <div class="flex-1 overflow-y-auto p-5 sm:p-6 space-y-6">
            ...
            <div class="space-y-3">
              <div 
                ...
                :key="item.id"
                class="bg-slate-50 p-3.5 rounded-2xl border border-slate-100 flex justify-between items-center"
              >
                <div class="flex items-center space-x-2.5 flex-1 pr-2">
                  <img v-if="item.image_url" :src="item.image_url" class="w-10 h-10 rounded-xl object-cover border border-slate-200 shrink-0">
                  <div>
                    <h4 class="font-black text-slate-800 text-xs leading-tight">{{ item.name }}</h4>
                    <span class="text-[11px] font-bold text-[#00880F]">{{ formatRupiah(item.price) }}</span>
                  </div>
                </div>

                <div class="flex items-center space-x-2">
                  <button @click="decreaseQty(index)" class="w-6 h-6 bg-white border border-slate-200 rounded-lg font-bold text-xs hover:bg-slate-100">-</button>
                  <span class="font-black text-xs text-slate-800 w-4 text-center">{{ item.quantity }}</span>
                  <button @click="increaseQty(index)" class="w-6 h-6 bg-white border border-slate-200 rounded-lg font-bold text-xs hover:bg-slate-100">+</button>
                  <button @click="removeItem(index)" class="text-rose-500 hover:text-rose-700 p-1 text-xs font-bold">🗑️</button>
                </div>
              </div>

              <div v-if="cart.length === 0" class="text-center py-12 text-slate-400 font-bold text-xs italic">
                Keranjang masih kosong. Silakan klik produk di etalase.
              </div>
            </div>
