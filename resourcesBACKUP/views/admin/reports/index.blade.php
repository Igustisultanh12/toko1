        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    {{-- Form Filter --}}
                    <form action="{{ route('admin.reports.index') }}" method="GET" class="border p-4 rounded-md mb-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                            <div>
                                ...
                                <select name="period" id="period" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                    <option value="daily" @if(request('period', 'daily') == 'daily') selected @endif>Harian</option>
                                    <option value="weekly" @if(request('period') == 'weekly') selected @endif>Mingguan</option>
                                    <option value="monthly" @if(request('period') == 'monthly') selected @endif>Bulanan</option>
                                    <option value="yearly" @if(request('period') == 'yearly') selected @endif>Tahunan</option>
                                </select>
                            </div>
                            <div id="date-input">
                                <label for="date" class="block font-medium text-sm text-gray-700">Tanggal:</label>
                                <input type="date" name="date" value="{{ request('date', now()->format('Y-m-d')) }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
                            </div>
                            <div id="month-input" style="display: none;">
                                <label for="month" class="block font-medium text-sm text-gray-700">Bulan:</label>
                                <input type="month" name="month" value="{{ request('month', now()->format('Y-m')) }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
                            </div>
                            <div id="year-input" style="display: none;">
                                <label for="year" class="block font-medium text-sm text-gray-700">Tahun:</label>
                                <input type="number" name="year" value="{{ request('year', now()->format('Y')) }}" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" placeholder="Contoh: 2025">
                            </div>
                    ...
                    <!-- Tombol Cetak Struk Harian diletakkan di sini -->
                    <div class="mb-6">
                        <a href="{{ route('admin.reports.print.daily') }}" 
                           target="_blank" 
                            ...
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200 border">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        ...
                                        @empty
                                        <tr>
                                            <td colspan="5" class="px-6 py-4 text-center">Tidak ada data penjualan untuk periode ini.</td>
                                        </tr>
                                        @endforelse
                                    </tbody>
                                    ...
                                    <tfoot class="bg-gray-50">
                                        <tr>
                                            <th colspan="4" class="px-6 py-3 text-right text-sm font-medium text-gray-900">Total Keseluruhan</th>
